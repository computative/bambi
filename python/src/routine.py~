    ...
    for C in CList: # for each cube
        ...
        if not any(np.array([Ak.In(C) for Ak in A])):
            ...
            for V, flag in zip(C.vertices,C.signature):
                sgn = 1-2*flag
                FMaxTemp = cy.F(V,A) + np.maximum(0, sgn[0]*s[0]*H[1-flag[0]][:,0] ) + \
                                       np.maximum(0, sgn[1]*s[1]*H[1-flag[1]][:,1] ) + \
                                       np.maximum(0, sgn[2]*s[2]*H[1-flag[2]][:,2] )
                FMinTemp = cy.F(V,A) + np.minimum(0, sgn[0]*s[0]*H[ flag[0] ][:,0] ) + \
        ...
        if (not any(FMin*FMax>0)):
            if Itrs - i > 1:
                ...
                # subdivide
                CListNext.extend(
                    [cy.Cell(np.array([_5a,_5b,c1]),np.array([a2,b2,_5c])),
...
roots, vals = cy.prune(vertices,A)
print("Zeros found:")
print(roots)
